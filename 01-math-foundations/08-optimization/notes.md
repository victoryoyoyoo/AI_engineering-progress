...
> 容易誤會成:Adam結合了momentum跟自適應學習率,理論上比SGD更「進步」,所以正式訓練時一定該選Adam。
>
...
> 容易誤會成:只要學習率設得夠小,梯度下降就一定能找到全域最小值(global minimum)。
>
...
`history.append(params)` 存進去的只是指向同一個 list 的參照,之後 `params` 被原地修改,`history` 裡所有紀錄會一起變。要存目前的狀態,寫 `params[:]` 複製一份。這堂課的 `step()` 因為回傳全新的 list 所以不會真的出事,`params[:]` 是防禦性寫法。
