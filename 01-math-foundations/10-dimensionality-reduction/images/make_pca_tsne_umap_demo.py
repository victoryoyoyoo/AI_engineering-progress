...
axes[0].set_title("PCA\n(線性,保留全域變異方向)", fontsize=11.5, fontweight="bold")

axes[1].scatter(X_tsne[:, 0], X_tsne[:, 1], c=y, cmap=cmap, s=12, alpha=0.7)
axes[1].set_title("t-SNE\n(保留鄰居關係,群更緊實分開)", fontsize=11.5, fontweight="bold")

axes[2].scatter(X_umap[:, 0], X_umap[:, 1], c=y, cmap=cmap, s=12, alpha=0.7)
axes[2].set_title("UMAP\n(類似t-SNE但更快、更保留全域結構)", fontsize=11.5, fontweight="bold")

for ax in axes:
    ax.set_xticks([])
    ax.set_yticks([])

fig.suptitle("同一份20維、5群的資料,降到2維:三種方法的取捨", y=1.02)
plt.tight_layout()
