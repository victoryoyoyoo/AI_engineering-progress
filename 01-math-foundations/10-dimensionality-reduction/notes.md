...
<details>
<summary>PCA的`fit`方法,一步一步拆解</summary>
...
**`np.cov` 的 `rowvar` 設反時,shape 會變成樣本數 × 樣本數**

3 個特徵的共變異數矩陣應該是 3×3;如果印出 `(500,500)`,就代表 `rowvar` 設反了,numpy 把 500 筆樣本當成 500 個變數。先用特徵數推論出應有的 shape,再印 `cov_matrix.shape` 對照。

**平方是為了不讓正負抵消,不是為了保留原始尺度**

重建誤差要平方,是避免正負差互相抵消。想回到原始尺度,最後再開根號得到 RMSE。
...
- **rank(秩)與資料的有效維度是同一個直覺**:PCA能用少數幾個主成分保留大部分變異量,靠的是「資料維度間相關性/冗餘越高,少數主成分就能抓住大部分變異量」 → 這個直覺第一次出現在 [Lesson 1](../01-linear-algebra-intuition/notes.md#30秒抓重點),那邊的rank(矩陣裡線性獨立的方向數)量的是同一件事:矩陣表面維度很大,但冗餘的列不增加真正的維度,LoRA就是利用這個特性省參數,PCA則是用變異量這個更貼近真實資料分佈的判準做同樣的事
...
<details>
<summary>Q1: PCA做完降維後,發現用不同套件、不同次執行,算出來的主成分方向正負號不一樣,這是bug嗎?</summary>

不是bug,是eigenvector本身的數學性質。eigenvector的方向定義本身正負號是任意的——如果`v`滿足`Av=λv`,那`-v`同樣滿足這個式子,兩者代表的是完全相同的「方向」,只是箭頭指的方向相反,在幾何上是同一條線。不同函式庫、甚至同一個函式庫不同次執行(取決於底層數值算法的實作細節),都可能給出正負號相反的主成分向量,這是正常現象,不代表算錯。另外要留意的是,當兩個相鄰的eigenvalue非常接近時,對應的eigenvector方向本身會對資料裡的微小數值雜訊變得敏感、不太穩定,這時候比較不同工具算出來的PCA結果,不能直接比對某個主成分的正負號或些微方向差異,而是要看降維後保留的變異量(explained variance ratio)、或者重建誤差這類跟正負號無關的整體指標是不是一致。

</details>

<details>
<summary>Q2: 資料集裡有兩群資料是同心圓分布,你會用標準PCA還是Kernel PCA?為什麼?</summary>

會用Kernel PCA。標準PCA只能在原始空間裡找一條「直線方向」,讓資料在這個方向上的變異量最大——但同心圓資料(內圈一群、外圈一群)不管挑哪個方向的直線去投影,兩群的資料點都會混在一起,實測跑出來兩個主成分的變異量也幾乎一樣(沒有哪個方向特別重要),PCA找不到能把兩群分開的線性方向。Kernel PCA的做法是不直接在原始低維空間找方向,而是透過RBF核函數(`exp(-gamma*距離平方)`)算出資料點兩兩之間的相似度,這個相似度矩陣等價於把資料投影到一個隱含的高維空間再做線性PCA(kernel trick的巧妙之處是不用真的算出高維座標,只需要知道任兩點在高維空間裡有多相似)。原本在低維空間裡糾纏在一起、線性方法分不開的同心圓結構,在這個隱含的高維空間裡可能就能找到一個能把兩群分開的方向,這正是Kernel PCA能處理標準PCA處理不了的非線性資料結構的原因。

</details>

<details>
<summary>Q3: 為什麼KNN這類依賴距離的演算法,在特徵維度很高時效果會明顯變差?</summary>

距離公式是`sqrt(每個維度差的平方和)`,維度越多,相當於加總越多個獨立的隨機差值。這跟大數法則是同一套邏輯:丟一次骰子結果差異很大,但丟很多次取平均,平均值會很穩定地收斂到某個值,不容易出現極端結果——維度增加對距離做的是同一件事,每多一個維度就像多丟一次骰子,維度越多,每個資料點的總距離就越被拉向同一個穩定值,原本「近的點」跟「遠的點」之間的差距會逐漸消失,所有點看起來都差不多遠。KNN、聚類這類演算法的判斷基礎完全建立在「距離近=相似,距離遠=不相似」這個前提上,一旦維度高到讓所有點的距離都趨於相同,「近」跟「遠」本身就失去了區分能力,模型自然找不到真正有意義的鄰居或群集,這也是實務上遇到高維特徵時,常常要先做降維(例如PCA)再套用這類依賴距離的演算法的原因。

</details>

<details>
<summary>Q4: 上線後要對一筆新資料做PCA降維,為什麼不能用這筆新資料自己重新算一次mean再置中?</summary>

PCA留下的k個方向(components),是根據訓練資料置中後的座標系去算出來的——也就是說,這些方向的定義本身就跟訓練資料的mean綁在一起,是「以訓練資料的平均值為原點」這個座標系底下的方向。如果對新資料改用它自己的mean去置中,等於是站在一個不同的原點上,卻硬套用根據訓練資料座標系算出來的方向去做投影,兩者的座標系對不齊,算出來的降維結果沒有意義,而且新資料如果只有少量幾筆甚至一筆,自己算出來的mean本身統計上也很不穩定、不可靠。正確做法是`transform`固定使用`fit`時存下的訓練資料mean,不管吃到的是原本那批資料還是全新的資料,永遠用同一個座標系原點去置中,這樣新資料投影出來的結果才能跟訓練資料當初學到的主成分方向保持一致、可以互相比較。
...
<details>
<summary>Q1: 第1題:PCA前3個主成分的explained variance ratio是`[0.95, 0.03, 0.02]`,你會留幾維?為什麼?</summary>
...
<details>
<summary>Q2: 第2題:同心圓資料,標準PCA為什麼分不開兩群?Kernel PCA靠什麼機制能分開?</summary>
...
<details>
<summary>Q3: 第3題:維度詛咒具體是什麼現象?會讓哪類演算法失效,為什麼?</summary>
