...
ax.set_title("原始分布:單次擲骰(均勻分布,完全不是鐘形)", fontsize=11.5, fontweight="bold")
ax.set_xlabel("點數")

ax = axes[1]
ax.hist(averages_of_30, bins=30, color="#4C72B0", alpha=0.75, edgecolor="black")
ax.set_title("30個一組取平均,重複3000組:趨近常態分布", fontsize=11.5, fontweight="bold")
ax.set_xlabel("平均值")

fig.suptitle("中央極限定理(CLT):不管原始分布長什麼樣,樣本平均值都趨近常態分布")
plt.tight_layout()
