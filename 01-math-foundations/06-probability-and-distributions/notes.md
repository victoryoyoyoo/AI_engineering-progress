...
<details>
<summary><b>訓練語言模型算一整句話的機率時,為什麼要用「log機率相加」而不是把每個詞的機率直接連乘?</b></summary>
...
<details>
<summary><b>模型某次forward pass的softmax輸出出現NaN,可能的原因跟修法是什麼?</b></summary>
...
<details>
<summary><b>訓練途中cross-entropy loss突然變成`inf`,可能是哪個環節出了問題?</b></summary>
...
兩個半小時。(課程建議時間:約75分鐘,實際花費是建議時間的2倍)

## 課程結尾理解確認題(先自己想過一遍,再點開看答案,這樣才是真的在複習)

<details>
<summary><b>Q1：PDF(機率密度函數,Probability Density Function)在某一點的值,為什麼可以大於1?這是不是代表機率超過了100%?</b></summary>
...
<details>
<summary><b>Q2：softmax 為什麼要先減掉最大的 logit 再取 exp?這個技巧為什麼在數學上等價於沒有做這個變換之前的結果?</b></summary>
...
<details>
<summary><b>Q3：訓練語言模型時,為什麼要用「log 機率相加」而不是直接把每個詞的機率「連乘」?</b></summary>
