...
<details>
<summary>醫療檢測範例——反直覺結論的具體算法</summary>
...
<details>
<summary>Naive Bayes分類器邏輯</summary>
...
<details>
<summary>為什麼「L2正則化本質上是貝氏統計」——完整推導</summary>
...
<details>
<summary>共軛先驗(Conjugate Prior)跟Beta分布</summary>
...
>
> 實際上:這兩個條件機率一般完全不相等,這正是貝氏定理存在的理由。這堂課的罕見疾病檢測就是活生生的例子:P(陽性|生病)高達99%,但P(生病|陽性)卻不到1%,兩者差了快100倍——把兩者搞混,會直接導致「篩檢陽性=幾乎確定生病」這種嚴重誤判。

**99% 準確的檢測,陽性也幾乎不代表生病**

10,000 人中只有 1 人得病、測試準確率 99%(誤判率 1%)時,健康的 9,999 人裡約有 100 人被誤判成陽性,遠多於真病人的 1 人。不是準確率不準,是健康人口的基數太大,1% 乘上一個巨大數字,結果依然很大,這叫 base rate fallacy。

**巢狀 defaultdict 的外層要用 lambda**

外層要寫成 `defaultdict(lambda: defaultdict(int))`。直接把 `defaultdict(int)` 當外層的預設值,所有類別會共用同一個內層字典,是常見的 bug 陷阱。
...
<details>
<summary>Q1: 一個罕見疾病篩檢工具號稱準確率99%,業務想直接拿「陽性=確診」來做決策,你會怎麼解釋為什麼不能這樣用?</summary>

用具體人數來解釋最有說服力:假設1萬人中只有1人真的得病、測試準確率99%(1%誤判率)。10,000人裡,真病人1位幾乎一定測出陽性(0.99人),但健康的9,999人裡也會有大約1%被誤判成陽性(約100人)。所有測出陽性的人加起來約101人,其中真正有病的只有不到1個人,換算下來,陽性的人真正得病的機率只有大約0.98%,遠遠不到「確診」的程度。原因在於健康人口的基數(9,999)遠遠大於病人基數(1),就算誤判率只有1%這麼低,乘上這麼大的健康人口基數,絕對誤判人數(約100人)還是遠遠超過真正的病人數——不是準確率不準,是分母(健康人口)太大。這叫base rate fallacy(基本比率謬誤),只看準確率、忽略「這個病本身有多罕見」(也就是先驗機率)是很容易踩的統計陷阱。實務上正確的做法是把單次陽性當成一個「需要進一步確認」的訊號,而不是最終結論——用貝氏方式,拿第一次的後驗機率當第二次檢測的先驗,做第二次確認測試,兩次合併計算後的後驗機率才會夠高,值得拿來做決策。

</details>

<details>
<summary>Q2: 為什麼可以說「訓練時加L2正則化(weight decay)」等於「假設權重有一個以0為中心的高斯先驗」?這個連結對設定正則化強度λ有什麼啟發?</summary>

MAP估計要最大化的目標是`P(資料|w)·P(w)`,取負log後等於最小化`-log P(資料|w) - log P(w)`。如果假設w的先驗是以0為中心的高斯分布,`-log P(w)`這一項展開後(去掉跟w無關的常數項)剛好等於`w²/(2σ²)`,所以MAP要最小化的目標變成`loss(資料|w) + w²/(2σ²)`——這跟L2正則化(ridge regression)的損失函數`loss(資料|w) + λ·w²`形狀完全一樣,只是`λ`對應到`1/(2σ²)`,是同一條公式的兩種語言講法。這個連結對設定λ的啟發是:λ不是一個純粹靠試出來的超參數,它背後代表的是「你對權重應該多接近0有多大的信心」——λ越大,等同於σ²越小,代表你的先驗信念很強、很確定權重應該集中在0附近,不容許權重偏離太多;λ越小,等同於σ²越大,代表先驗信念比較弱,願意讓資料主導、權重可以離0比較遠。實務上調λ時可以用這個角度去想:資料量少、擔心過擬合時,可以想成先驗信念比較重要,適合用較大的λ;資料量夠大時,資料本身的證據力已經能蓋過先驗,λ可以設小一點,不用強行把權重往0拉。

</details>

<details>
<summary>Q3: Naive Bayes分類器遇到訓練資料裡沒出現過的詞,為什麼不能直接讓那個詞的機率算成0?</summary>

如果某個詞在訓練資料裡從沒在某個類別出現過,MLE會直接把`P(詞|類別)`算成0,因為觀察到的次數就是0。但Naive Bayes分類時是把所有特徵的機率取log後相加(或原始機率連乘)——只要這串連加(或連乘)裡有任何一項是`log(0)`(負無窮)或`0`(連乘的情況),不管句子裡其他詞多麼強烈指向這個類別,整個分數都會被這一個沒見過的詞直接拖到負無窮或0,等於讓分類器對這個類別判死刑,完全忽略了句子裡其他所有真正有用的證據。這在實務上很容易發生,因為訓練資料不可能涵蓋所有可能出現的詞,新句子裡出現一個訓練時沒看過的詞是很正常的事。解法是Laplace smoothing(拉普拉斯平滑):算`P(詞|類別)`時,分子分母都加上一個小常數(通常是1),保證任何詞、任何類別的機率永遠不會是精確的0,但當這個詞出現次數夠多時,平滑帶來的影響會被稀釋到可以忽略,不會扭曲原本統計出來的機率結構。

</details>

<details>
<summary>Q4: 什麼情境下你會特別偏好用共軛先驗(conjugate prior)而不是任意先驗分布做貝氏更新?</summary>

共軛先驗最大的優勢是更新公式會變成單純的加法(例如Beta先驗配上二項觀察,後驗直接是`Beta(a+成功次數, b+失敗次數)`),不需要做任何積分或數值近似,而且不需要保留原始的觀察資料,只要記住幾個描述分布的參數(例如Beta分布的a、b兩個數字),新資料進來時直接加上去就是新的後驗。這種特性特別適合線上學習(online learning)、串流資料處理這類情境:資料是持續不斷進來的,不可能每次都把歷史所有資料重新拿出來算一次後驗,共軛先驗讓「今天的後驗變成明天的先驗」這件事計算成本極低、幾乎即時。實務應用像Thompson sampling(bandit演算法選擇要不要繼續嘗試某個選項)、串流異常偵測、A/B測試的貝氏版本,都需要頻繁地、即時地根據新資料更新信念,這正是共軛先驗最適合發揮的場景;反過來,如果更新頻率不高、算力不是問題,或者手上真正的先驗知識就是某個沒有共軛形式的分布(硬套一個不符合實際信念的共軛先驗反而失真),就沒有必要為了計算方便犧牲先驗的準確性,改用數值方法(如MCMC)近似計算後驗也是合理選擇。
...
<details>
<summary>Q1: 貝氏定理(Bayes' Theorem)是怎麼從條件機率(conditional probability)的兩種寫法推導出來的?</summary>
...
<details>
<summary>Q2: 為什麼一個測試準確率99%的疾病檢測,在「1萬人中只有1人得病」的罕見疾病情境下,驗出陽性的人真正得病的機率卻不到1%?這個現象背後的道理是什麼?</summary>
...
<details>
<summary>Q3: 為什麼可以說「MAP估計搭配一個以0為中心的高斯先驗」,在數學上跟訓練時加L2正則化(weight decay)完全等價?</summary>
...
- `bayes()` 函式 — 逐行對照公式手打進 `practice.py`
- `NaiveBayes` 類別 — 沒有手打,只在 `reference.py` 留了一份參考。這是真實缺口,已記進 `review-queue.md`,之後要補成自己手打的版本
