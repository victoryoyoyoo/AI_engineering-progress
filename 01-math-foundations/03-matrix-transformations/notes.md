...
**Q1：什麼是 eigenvector(特徵向量)跟 eigenvalue(特徵值)?從定義式 `Av=λv` 出發,怎麼一步步推導出 characteristic equation `det(A-λI)=0`?**
...
**Q2：為什麼 RNN(循環神經網路)的權重矩陣,eigenvalue 絕對值大於1或小於1,會分別造成梯度爆炸或梯度消失?**
