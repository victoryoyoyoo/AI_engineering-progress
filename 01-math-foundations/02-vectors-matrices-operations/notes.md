# Lesson 2 - Vectors, Matrices & Operations(向量、矩陣與運算)
...
**形狀本來就一樣時,沒有 broadcasting 這回事**

broadcasting 只有在兩個陣列形狀不一樣、但可以延伸對齊時才會發生;形狀本來就相同,就是直接逐項運算。判斷有沒有用到 broadcasting,第一步是先看兩邊形狀是不是本來就一樣。從最後一個維度往前比,某個維度其中一邊是 1(或那個維度不存在)才會延伸;兩邊數字不相等又都不是 1,就直接報錯,不會硬猜。

## 相關概念(跨堂連結)

- **矩陣乘矩陣**:這堂課把 Lesson 1 的矩陣乘向量,擴展成矩陣乘矩陣 → [Lesson 1](../01-linear-algebra-intuition/notes.md)
- **list comprehension**:Lesson 1 學過基本語法,這堂課遇到雙層巢狀的版本,用 C++ 三層迴圈的矩陣乘法對照 → [Lesson 1](../01-linear-algebra-intuition/notes.md)
- **Broadcasting**:這堂課第一次遇到(bias 加到一批資料),Lesson 12 補完整規則,並練外積和三維的情況 → [Lesson 12](../12-tensor-operations/notes.md)

## 面試向問題

<details>
<summary>Q1: 生產環境跑出來的模型輸出全部一樣或出現NaN,你懷疑是不是把matrix multiply跟element-wise multiply搞混了,要怎麼從程式碼快速排查?</summary>
...
<details>
<summary>Q2: 為什麼神經網路一層要寫成`relu(W@x+b)`而不是只留`W@x`?拿掉relu會發生什麼事?</summary>
...
<details>
<summary>Q3: 訓練時一次丟一整個batch的資料進網路,bias是怎麼自動套用到整批資料上的,不用手動複製成batch大小?</summary>
...
<details>
<summary>Q1: element-wise multiplication(逐元素相乘)跟 matrix multiplication(矩陣乘法)差在哪裡?各自對形狀(shape)有什麼要求?</summary>
...
<details>
<summary>Q2: `relu(W @ x + b)` 這一行代表神經網路的一層,W、b、relu 三個部分各自扮演什麼角色?</summary>
...
<details>
<summary>Q3: broadcasting 最常在什麼情境下被觸發?判斷規則是什麼?</summary>
...
| 花費時間 | 這堂課總共專注 2:16:30,其中 AI 相關部分就是這 2:16:30(整段都算)。比 Lesson 1 的 6:35:45 快非常多,主要是 Top-Down 策略省下大量手刻時間,加上環境設定的一次性成本這堂課不用再付。(課程建議時間:約60分鐘,實際花費是建議時間的兩倍多) |
...
<details>
<summary>List comprehension 複習:matmul 那段對照 C++ 三層迴圈</summary>
...
<details>
<summary>NumPy 是什麼、跟純 Python 的差別</summary>
...
<details>
<summary>Broadcasting 實際運作的時機</summary>
...
<details>
<summary>`@staticmethod` 跟 `@property`:兩種不需要「先有物件」或「不用加括號」的方法</summary>
...
<details>
<summary>三元運算式(ternary expression):`X if 條件 else Y`</summary>
