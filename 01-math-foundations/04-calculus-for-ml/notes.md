...
- 訓練loss不再下降,梯度看起來也接近0,你會怎麼判斷模型是卡在局部最小值、鞍點,還是真的已經收斂?(對應「Hessian 矩陣」/「鞍點 Saddle Point」那段)
- 為什麼深度學習訓練幾乎不用牛頓法,清一色用一階方法(SGD、Adam這類)?(對應「梯度下降 vs 牛頓法 對照」那段)
- 學習率設太大跟設太小,分別會在訓練曲線上看到什麼徵兆?(對應「學習率 Learning Rate」那段)
