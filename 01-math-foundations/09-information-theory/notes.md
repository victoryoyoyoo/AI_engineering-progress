...
> 容易誤會成:KL divergence是在量兩個分布之間的「距離」,可以跟一般的歐式距離一樣使用,覺得`D_KL(P‖Q)`應該等於`D_KL(Q‖P)`。
>
> 實際上:KL divergence不對稱,這堂課已經親自算過`D_KL(P‖Q)≠D_KL(Q‖P)`,而且它也不滿足三角不等式,數學上不是一個真正的距離度量(metric),只能算是「用Q取代P要多付出的成本」這種單方向的量——交換P跟Q的角色,算出來的數字通常不一樣。

**Pearson 跟 Poisson 不是同一種東西**

測驗時搞混過一次。Pearson correlation 量的是兩個變數之間線性關係的強弱(關係型);Poisson distribution 是計數型的機率分布,描述單位時間內某件事發生幾次。兩者除了名字都以 P 開頭,沒有直接關係。
